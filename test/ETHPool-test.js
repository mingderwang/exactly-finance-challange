...
describe("ETHPool", function () {
  ...
  it("Should not distribute rewards if pool is empty", async function () {
    ...
    await expect(ethpool.connect(owner).depositRewards({ value: oneEther })).to
      .be.reverted;
  });
  ...
  it("Should allow depositing rewards from team member", async function () {
    ...
    const oneHundredAndfiftyEth = ethers.BigNumber.from(
    ...
    expect(newBalance2).to.be.within(
      balance2,
      balance2.add(oneHundredAndfiftyEth)
    ); // Original Balance + 150 ethers
  });
  ...
  it("Should total substract the withdraw value", async function () {
    const oneHundredEth = ethers.BigNumber.from("100000000000000000000");
    const twoHundredEth = ethers.BigNumber.from("200000000000000000000");
    const threeHundredEth = ethers.BigNumber.from("300000000000000000000");
    ...
    const finalTotal = await ethpool.total();

    expect(finalTotal).to.be.equal(0);
  });
});
